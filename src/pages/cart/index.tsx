...
const CartPage = () => {
  ...
  const sumCartTotal: number = cartProducts.reduce((sum, current) => 
    sum + ((current.quantity ?? 1) * current.recommendedRetailPrice), 0);

  //Considering all products in the cart have the same currency:
  const cartCurrency: string = cartProducts.length > 0 ? cartProducts[0].recommendedRetailPriceCurrency : "";
  ...
  return <Layout>
    ...
    {cartProducts.length ?
      ...
      <p className="max-w-2xl text-right">
        ...
        <span className='text-3xl font-bold'>{sumCartTotal.toFixed(2)} {cartCurrency}</span>
      </p>
    </div>
    :

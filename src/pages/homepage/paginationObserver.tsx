...
export const usePagination = () => {
  const [pagination, setPagination] = useState<PaginationState>(paginationStore.get());
    ...
    const decrementPageNum = () => {
      paginationStore.set({pageNum: pagination.pageNum > 1 ? pagination.pageNum-1 : 1});
    }

    const calculateTotalPages = (totalProducts: number) => {
      paginationStore.set({...paginationStore.get(), totalPages: Math.ceil(totalProducts/SIZE)})
    }
